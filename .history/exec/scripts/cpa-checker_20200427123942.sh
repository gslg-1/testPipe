...
cpa_start ()
{
    echo debug
    #cpa $1 $2 cpa-end
    /cygdrive/d/workspace/git/rodir/cpachecker/scripts/cpa.sh -valueAnalysis-NoCegar-join -setprop analysis.checkCounterexamples=false -setprop solver.solver=SMTInterpol -setprop cpa.predicate.encodeBitvectorAs=INTEGER -setprop cpa.predicate.encodeFloatAs=RATIONAL
}
#####################################################################################
